...
**가장 공들인 부분.** 오케스트레이션을 사용하면서도 API 호출을 최대한 줄이는 것이 가능한지에 공들였다. 그래서 매 단계마다 에이전트의 작업 분배 계획을 확인하고, 보고를 받고, 승인한 뒤에야 실행했다.
- 코퍼스 수집과 모의 테스트는 LLM 호출 0회로 끝냈다.
- 기획만 따로 돌려(9회, $0.005) 배정표를 먼저 읽었고, 배정이 엉뚱한 것을 보고 프롬프트를 고쳐 다시 기획했다.
- 본 실행 전에는 실행 수·예상 호출·비용 상한을 보고받고 승인했다. 효과가 없을 실행(validate 끔)은 빼게 했다.
- 결과가 이상할 때(대조군 예산 미소진, 도구 버그)는 원인을 보고받은 뒤 재실험 범위와 비용을 확인하고 승인했다.

그 결과 58실행을 약 $0.55로 끝냈다. 사전 상한은 $0.7이었다.

**아쉬운 부분.** 마지막에 작업을 합치는 부분에서 점검을 다소 허술하게 했다. 그래서 그 부분에서 만족도가 낮게 나온 것 같다(6절).
- 배정·파견·예산은 단계마다 확인했지만, 종합 방식("절 원고를 그대로 이어 붙이고 서론·결론만 쓴다")은 처음 설계대로 두고 따로 검토하지 않았다.
- 지표도 읽기 단계(커버리지·중복·인용)만 재고 있어서, 합친 결과물의 흐름이 목적에 맞는지는 직접 읽기 전까지 드러나지 않았다.
- 다음에는 계획 승인 단계에 **"어떻게 합칠 것인가"도 보고 항목으로 넣고**, 종합 결과를 직접 읽는 점검을 실행 직후에 두겠다.
